...
setup
{
    -- Cloudberry: pg_class has no relallfrozen column (that is PostgreSQL 18),
    -- so the all-visible and all-frozen page counts come from the visibility
    -- map itself.  Right after a VACUUM they are exactly what the VACUUM
    -- stores in pg_class, as it takes them from the visibility map too.
...
step s1_get_set_vm_flags_stats
{
    ...
    SELECT c.all_frozen > frozen_flag_count as relallfrozen, c.all_visible > all_visibile_flag_count as relallvisible
        FROM pg_visibility_map_summary('vestat') c, stats_state;

    UPDATE stats_state
        SET frozen_flag_count = c.all_frozen,
            all_visibile_flag_count = c.all_visible
        FROM pg_visibility_map_summary('vestat') c;
}
