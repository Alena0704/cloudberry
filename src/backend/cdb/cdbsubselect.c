...
#include "utils/fmgroids.h"		/* F_COUNT_ANY */
...
static int	add_join_rte(Query *parse, JoinType jointype);
...
convert_EXPR_to_join(PlannerInfo *root, OpExpr *opexp)
{
	...
	if (safe_to_convert_EXPR(sublink, &ctx1))
	{
		...
		if (use_left_join)
		{
			...
			/*
			 * An outer join needs a range table entry of its own: nullability
			 * is tracked per outer join, and deconstruct_recurse() registers a
			 * join in root->outer_join_rels -- and marks the rels it nulls --
			 * only when the JoinExpr carries an rtindex.  Without one the
			 * planner never learns the pulled-up subquery is nullable here, so
			 * it pushes the comparison below the join and then drops the join
			 * as useless, losing the qual with it.
			 */
			joinRteIndex = add_join_rte(root->parse, JOIN_LEFT);
			join_expr->rtindex = joinRteIndex;

			flagVar = (Var *) makeVar(rteIndex, flagTLE->resno, BOOLOID, -1,
									  InvalidOid, 0);
			caseExpr = build_match_flag_case_expr(flagVar, aggVar, defaultExpr);

			/*
			 * The comparison is evaluated above the join (see
			 * pull_up_sublinks_qual_recurse), where the subquery's columns are
			 * null-extended for a no-match row.  Record that in the Vars, so
			 * the qual is recognized as belonging above the join rather than
			 * being pushed down into it.
			 */
			caseExpr = (Expr *) add_nulling_relids((Node *) caseExpr,
												   bms_make_singleton(rteIndex),
												   bms_make_singleton(joinRteIndex));

			list_nth_replace(opexp->args, 1, caseExpr);
		}
		else
		{
...
/*
 * add_join_rte
 *		Add a range table entry standing for a planner-created join.
 *
 * Outer joins have to be identifiable by an RT index: that index is what
 * ends up in root->outer_join_rels and in the varnullingrels of Vars the
 * join can null.  Inner and semi joins need no entry, so this is only for
 * the outer ones we synthesize.
 *
 * joinaliasvars is left empty.  It exists so that Vars referencing the join
 * RTE can be flattened back to the underlying columns, and nothing refers to
 * this join that way -- the Vars we emit point at the base relations and
 * merely carry the join's index in varnullingrels.
 */
static int
add_join_rte(Query *parse, JoinType jointype)
{
	...
	rte->joinleftcols = NIL;
	rte->joinrightcols = NIL;
	rte->join_using_alias = NULL;
	rte->alias = NULL;
