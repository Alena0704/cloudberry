 ...
 * and resgroupcmds.c), so the role must always be created with it.  The OID
 * is requested through the next_aux_pg_authid_oid override, which
 * GetNewOidForAuthId() honors for OIDs in the auxiliary range.  On a
